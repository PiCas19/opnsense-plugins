...
$(document).ready(function() {
    ...
    /**
     * Wait for bootgrid to be available before chaining
     */
    function initBootgridChain() {
        if (typeof std_bootgrid_reload !== 'undefined') {
            ...
            // Retry after 100ms if not available yet
            setTimeout(initBootgridChain, 100);
        }
    }

    // Initialize bootgrid chain
    initBootgridChain();
    ...
    mapDataToFormUI({'frm_GeneralSettings':"/api/deepinspector/settings/getGeneral"}).done(function(){
        ...
        console.error("Failed to load general settings:", textStatus, errorThrown);
    });
    ...
    mapDataToFormUI({'frm_ProtocolsSettings':"/api/deepinspector/settings/getProtocols"}).done(function(){
        ...
        console.error("Failed to load protocol settings:", textStatus, errorThrown);
    });
    ...
    mapDataToFormUI({'frm_DetectionSettings':"/api/deepinspector/settings/getDetection"}).done(function(){
        ...
        console.error("Failed to load detection settings:", textStatus, errorThrown);
    });
    ...
    mapDataToFormUI({'frm_AdvancedSettings':"/api/deepinspector/settings/getAdvanced"}).done(function(){
        ...
        console.error("Failed to load advanced settings:", textStatus, errorThrown);
    });
