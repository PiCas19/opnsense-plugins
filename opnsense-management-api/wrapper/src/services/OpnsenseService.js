...
class OpnsenseService {
        ...
        try {
          ...
          return {
            ...
            ssl_config: {
              ...
              ca_loaded: !!config.ssl.ca,
              client_cert_loaded: !!config.ssl.cert
            }
          };
        } catch (error) {
          lastError = error;
  ...
  /**
   * Formatta porta per OPNsense
   */
  formatPort(addressConfig) {
    if (!addressConfig || !addressConfig.port) {
      return '';
    }
  ...
  validateRuleData(ruleData) {
    ...
    // Valida configurazioni indirizzi
  ...
  /**
   * Reset rate limiting per operazione
   */
  ...
  /**
   * Ottieni backup configurazione
   */
  async getConfigBackup() {
    try {
      const response = await this.makeApiRequest('GET', '/api/core/backup/downloadBackup', {}, 'backup_config');
  ...
  async getInterfaces() {
    try {
      const response = await this.makeApiRequest('GET', '/api/diagnostics/interface/getInterfaceConfig', {}, 'get_interfaces');
  ...
  /**
   * Statistiche sistema OPNsense
   */
  async getSystemStats() {
    try {
      const response = await this.makeApiRequest('GET', '/api/diagnostics/system/systemHealth', {}, 'system_stats');
